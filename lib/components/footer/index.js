...
const { resolveRoute } = require('../base')
  ...
  createElement (props) {
    return html`
      <footer class="Footer">
            ...
            ${props.partners.map(partner => html`
              <li class="Footer-partner">
                <a class="Footer-link Footer-link--partner" target="_blank" rel="noopener noreferrer" href="${partner.link.url}" title="${partner.label}">
                  <img class="Footer-logo" src="${partner.logo.url}" width="${partner.logo.dimensions.width}" height="${partner.logo.dimensions.height}" />
                </a>
              </li>
            `)}
          </ul>
          <ul class="Footer-nav">
            ${props.menu.map(item => html`
              <li class="Footer-page">
                <a class="Footer-link Footer-link--primary" href="${resolveRoute(this.state.routes.page, item.params)}">${item.label}</a>
                ${item.sections.length ? html`
                  <ul class="Footer-sections">
                    ${item.sections.map(section => html`
                      <li class="Footer-section">
                        <a class="Footer-link" href="${resolveRoute(this.state.routes.section, section.params)}">${section.label}</a>
                      </li>
                    `)}
                  </ul>
                ` : null}
        ...
        <svg class="Footer-logo Footer-logo--qp" width="31" height="54" viewBox="0 0 31 54" role="presentational">
