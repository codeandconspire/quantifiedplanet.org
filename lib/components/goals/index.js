...
module.exports = class Goals extends Component {
  constructor (emitter) {
    ...
    if (typeof window !== 'undefined') {
      this.onresize = nanoraf(() => {
        ...
        this.narrow = this.vw < 768
        this.medium = this.vw >= 768 && this.vw < 1024
        this.wide = this.vw >= 1024
      })
  ...
  onscroll (event) {
    ...
    if (this.wide) {
      const px1 = modulate(scroll, [start, stop], [0, 150], true)
      const px2 = modulate(scroll, [start, stop], [0, -150], true)
