...
module.exports = class LineChart extends Component {
  constructor (opts) {
    ...
    super(`line-chart-${(new Date() % 9e6).toString(36)}`)
  ...
  init () {
    if (!this.data) {
      window.fetch([
        ...
        `metric=${this.type}`,
        `city=${this.city}`,
        `start=${(new Date(Date.now() - 1000 * 60 * 60 * 24 * 7)).toJSON()}`,
        `end=${(new Date()).toJSON()}`,
    ...
    this.chart.on('draw', data => {
      ...
      if (data.type === 'line') {
        ...
        data.group.getNode().appendChild(
          html`<circle class="LineChart-end" cx="${point.x}" cy="${point.y}" r="${END_RADIUS}" stroke="#56C0D5" stroke-width="3" fill="#141530" />`
        )
        data.group.getNode().appendChild(
          html`<text class="LineChart-label" x="${point.x - (END_RADIUS * 2)}" y="${data.axisY.axisLength + OFFSET / 2}">${text`7 days`}</text>`
        )
      }
  ...
  createElement () {
    ...
    return html`
      <div class="LineChart LineChart--${this.type}">
