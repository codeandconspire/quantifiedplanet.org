...
module.exports = class View extends Component {
  ...
  // create view wrapper
  // (str, fn, fn) -> fn
  static createView (name, getView, getTitle) {
    return function (state, emit, render) {
      if (state.error) getView = state => error(state.error)

      let children, title
      try {
        children = getView(state, emit, render)
        if (typeof getTitle === 'function') title = getTitle(state)
      } catch (err) {
