...
const { ROUTES, resolveRoute, className } = require('../base')
...
module.exports = class Header extends Component {
  ...
  update () {
    return this.href !== this.state.href
  }
  ...
  toDataExplorer (event) {
    const section = document.getElementById('data-explorer')
    if (!section) {
      this.emit('pushState', `${resolveRoute(ROUTES.homepage)}`)
    } else {
      document.getElementById('data-explorer').scrollIntoView()
    }
    event.preventDefault()
  }
      ...
      <header class="Header ${this.state.routeName !== 'homepage' ? 'Header--light' : 'Header--collapse'}">
        <nav class="Header-container">
          <a class="Header-home" href="${resolveRoute(ROUTES.homepage)}" onclick=${this.toHome}>
            ...
            <span class="u-hiddenVisually">Quantified Planet</span>
          </a>
          <ul class="Header-nav">
            ${this.state.meta.menu.map(item => {
              const href = resolveRoute(ROUTES.page, item.params)
              const isActive = href === resolveRoute(ROUTES[this.state.routeName], this.state.params)
              return html`
                <li class="Header-page">
                  <a class="${className('Header-link', {'is-active': isActive})}" href="${href}">
                    ${item.label}
                  </a>
                </li>
              `
            })}
            <li class="Header-page Header-page--fallback">
              <a class="Header-link" href="#footer" onclick=${this.toFooter}>
                ${text`Navigation`}
              </a>
            </li>
          </ul>
          <a class="Header-data" href="/#data-explorer" onclick=${this.toDataExplorer.bind(this)}>
