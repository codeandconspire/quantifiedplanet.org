...
module.exports = class BubbleChart extends Component {
    ...
    if (!this.data) {
      window.fetch([
        ...
        return response.json().then(data => {
          if (response.status !== 200 || !data.payload.length || data.status !== 'SUCCESS') {
  ...
  static error (err) {
    return html`
      <div class="BubbleChart is-error">
        <div class="BubbleChart-error">${text`Temporary issue loading data`}
