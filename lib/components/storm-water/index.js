...
module.exports = class StormWater extends Component {
  load (element) {
    ...
    background.appendChild(html`
      <animate attributeName="d" begin="0ms" dur="3000ms" fill="freeze" values="${[background.getAttribute('d')].concat(BACKGROUND_PATHS).join(';')}" />
    `)

    foreground.appendChild(html`
      <animate attributeName="d" begin="0ms" dur="3000ms" fill="freeze" values="${[foreground.getAttribute('d')].concat(FOREGROUND_PATHS).join(';')}" />
    `)
  }
