...
module.exports = class Story extends Component {
  ...
  load () {
    ...
    this.introWrap = this.element.querySelector('.js-introWrap')
  ...
  onscroll (event) {
    ...
    if (scroll < introDuration) {
      ...
      this.introWrap.style.visibility = ''
      ...
      if (scroll < slideDuration) {
        ...
        this.introWrap.style.opacity = 1
        ...
        const fade = modulate(scroll, [(width + (height / 3)), width], [0, 1], true)
        this.introWrap.style.opacity = fade.toFixed(3)
        this.overlay.style.transform = `translateX(${width * -1}px)`
        this.blueprint.style.transform = `translateX(${width}px)`
      }
    } else {
      // at system
      this.introWrap.style.visibility = 'hidden'
  ...
  createElement (doc) {
    ...
    return html`
      <div class="${className('Story', {'has-js': typeof window !== 'undefined', 'has-appeared': this.appeared})}" id="${this._name}">
        <div class="Story-introWrap js-introWrap">
