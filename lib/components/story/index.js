...
module.exports = class Story extends Component {
  constructor (id) {
    ...
    this.state = {}
    this.sections = [ System ].map((Section, index) => {
  ...
  load (doc) {
    ...
    this.content.style.paddingTop = `${this.dimensions.height}px`
  ...
  onscroll (event) {
    ...
    for (let i = 0, len = this.sections.length, inview, section; i < len; i++) {
      section = this.sections[i]
      inview = (
        ((section.top + height) < (scroll + height)) &&
        ((section.top + section.height + height) > scroll)
      )
    ...
    return html`
      <div class="Story" id="${this._name}">
        <div class="Story-intro js-intro" id="${this._name}-intro">
          ...
          <video class="Story-planet" src="/out.webm" preload="auto" role="presentation" autoplay playsinline muted loop></video>
