...
module.exports = class Story extends Component {
  ...
  onresize () {
    ...
    const firstFrame = width < 768 ? 0 : this.element.querySelector('.js-firstFrame').offsetTop
    const introDuration = (width + height) - firstFrame

    this.dimensions = { width, height, isLandscape, slideDuration, introDuration }
  ...
  onscroll (event) {
    ...
    const { width, height, isLandscape, slideDuration, introDuration } = this.dimensions

    // at intro
    if (scroll < introDuration) {
      const value = isLandscape ? (height * 2) : (height * 2)
      const intro = modulate(scroll, [value, 0], [height * -0.1, 0], true)
  ...
  onended () {
    if (this.videosLoaded === 1) {
      ...
      this.videos[1].play()
      this.videos[1].classList.remove('is-hidden')
      this.videos[0].pause()
      this.videos[0].remove()
      this.unbindVideo()
    }
  }
    ...
    return html`
      <div class="${className('Story', {'has-js': typeof window !== 'undefined', 'has-appeared': this.appeared})}" id="${this._name}">
        <div class="Story-intro js-intro" id="${this._name}-intro">
          ${this.videosLoaded !== 2 ? html`
            <video class="Story-planet js-video" preload="auto" playsinline muted role="presentation" width="200" height="200">
              <source src="/short.webm" type="video/webm" />
            </video>
          ` : null}
          <video class="Story-planet js-video ${this.videosLoaded !== 2 ? 'is-hidden' : ''}" preload="metadata" playsinline muted loop role="presentation" width="200" height="200">
            <source src="/long.webm" type="video/webm" />
          </video>
          ...
          <div class="Story-block Story-block--dark Story-block--outro">
            <div class="View-container">
              <div class="Story-copy">
                <div class="Text">
                  <h2 class="Text-story5">Quick Outro</h2>
                  <p class="Text-story Text-story--center">Morbi lorem mauris, bibendum ut laoreet ac, placerat id mi. Proin laoreet, sapien in molestie finibus, lorem ipsum feugiat lectus, vel fringilla nunc odio sed nunc.</p>
                </div>
              </div>
            </div>
          </div>
