...
const { text, linkResolver } = require('../components/base')
      ...
      <div class="Text">
        ${doc.data.body.map(slice => {
          switch (slice.slice_type) {
            ...
            case 'preamble': return asElement(slice.primary.text, linkResolver(state.routes), (element, content, children) => {
              return (element.type === Elements.paragraph) ? html`
                <p class="Text-preamble">${children}</p>
              ` : null
            })
            case 'text': return asElement(slice.primary.body, linkResolver(state.routes))
            case 'blockquote': return html`
              <blockquote>
                ${asElement(slice.primary.text, linkResolver(state.routes))}
