...
module.exports = class Page extends Component {
  ...
  update (state) {
    ...
    const newContent = this.isLoading && typeof doc !== 'undefined'
  ...
  afterupdate () {
    ...
    if (doc && this.isLoading) {
      const el = document.getElementById(this.state.params.section)
      if (el) window.scrollTo(0, el.offsetTop)
    }

    this.isLoading = !doc
    this.section = state.params.section
  }
}
      ...
      <div class="View-switcheroo">
        ...
        <nav class="View-menu">
          ${menu(doc.data.body.filter(slice => slice.slice_type === 'heading').map(slice => {
            ...
            const href = resolveRoute(state.routes.section, { page: doc.uid, section: section })
            const onclick = () => scrollToAnchor(`#${section}`, { block: 'start', behavior: 'smooth' })
